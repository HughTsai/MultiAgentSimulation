...
public class AgentWorldSimpleTest extends AgentsWorld{
	...
	//该次实验中，alpha上限，下限，增量
	private double eUpper;
	private double eLower;
	private double eIncrement;
	//该次实验的agent数量
	...
	public AgentWorldSimpleTest(double eUpper, double eLower, double eIncrement,
			int agentNum) {
		super();
		this.eUpper = eUpper;
		this.eLower = eLower;
		this.eIncrement = eIncrement;
...
//		agents[49] = new FirmAgent("Agent 49", u, aerfa, 50, e, k);
	 ...
	 * 破产传递的“一次”模拟（只适用于实验5）
	 * @param num 该次模拟的序号
	 * @param currentEValue 该次模拟的α值
	 */
	private void startSimulation(int num ,double currentEValue) {
		// TODO 自动生成的方法存根
		...
		timestep = 0;
		System.out.println("当前e值为："+currentEValue+":第"+(num+1)+"次模拟结束");
	}
	/**
	 * 实验模拟
	 * @param simulateNum 每个e值需要模拟的次数
...
//		}
		//让e值从最小值一直增加到最大值
		for(double eValue = this.eLower;eValue <= this.eUpper;eValue += this.eIncrement){
			eValue = this.round(eValue, 3);
			eRstData = new RecoverRstData(eValue);
			//一个α值，进行多次模拟
			for(int i = 0;i < simulateNum;i++){
				this.init(0.3, 0.5, eValue, 10);
				...
				startSimulation(i, eValue);
				eRstData.addData(AgentsWorld.bankruptNum);
				
				//Logger.log_endSimulation();//日志结束记录
			}
			results.addData(eRstData);
		}
		Logger.stop();
